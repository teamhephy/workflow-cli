...
func (d *HephyCmd) ServicesList(appID string) error {
...
func (d *HephyCmd) ServicesAdd(appID, procfileType string, pathPattern string) error {
	s, appID, err := load(d.ConfigFile, appID)
...
func (d *HephyCmd) ServicesRemove(appID, procfileType string) error {
	s, appID, err := load(d.ConfigFile, appID)
